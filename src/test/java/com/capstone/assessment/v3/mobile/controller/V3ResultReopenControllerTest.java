...
import com.capstone.assessment.v3.mobile.service.V3ResultReopenService;
...
class V3ResultReopenControllerTest {
    final ObjectMapper mapper=new ObjectMapper().findAndRegisterModules();
    final V3ResultReopenService service=mock(V3ResultReopenService.class);
    final V3AuthenticatedUser teacher=new V3AuthenticatedUser(42,"S1","","teacher","active","");
    final String uuid="00000000-0000-4000-8000-000000000001",route="/api/v3/mobile/results/"+uuid+"/reopen";
    MockMvc mvc;
    @BeforeEach void setup(){
        mvc=MockMvcBuilders.standaloneSetup(new V3ResultReopenController(service,mapper)).setCustomArgumentResolvers(new AuthenticationPrincipalArgumentResolver()).setControllerAdvice(new V3AuthExceptionHandler()).build();
        SecurityContextHolder.getContext().setAuthentication(new UsernamePasswordAuthenticationToken(teacher,null,List.of()));
        when(service.reopen(eq(teacher),eq(uuid),any())).thenReturn(new V3LifecycleAck(uuid,"pending_verification",5,1,null,"created",Instant.parse("2026-09-12T00:00:00Z")));
    }
    @AfterEach void close(){SecurityContextHolder.clearContext();}
    String request()throws Exception{return Files.readString(Path.of("docs/contracts/mobile-v3/1.0.0/fixtures/proposed/reopen-request.json"));}
    ...
    @Test void acceptsExistingContractAndReturnsNoStore()throws Exception{
        mvc.perform(post(route).contentType(MediaType.APPLICATION_JSON).content(request())).andExpect(status().isOk())
                .andExpect(header().string("Cache-Control","no-store")).andExpect(jsonPath("$.data.resultStatus").value("pending_verification"));
        verify(service).reopen(eq(teacher),eq(uuid),any(V3ReopenRequest.class));
    }
    ...
    @Test void rejectsOversizedBody()throws Exception{
        mvc.perform(post(route).contentType(MediaType.APPLICATION_JSON).content(" ".repeat(V3ResultReopenController.MAX_BODY_BYTES+1))).andExpect(status().isPayloadTooLarge());verifyNoInteractions(service);
    }
