...
@RestController
@Profile("v3")
public class V3ResultReopenController {
    public static final int MAX_BODY_BYTES=32*1024;
    private final V3ResultReopenService service;
    private final ObjectMapper mapper;
    public V3ResultReopenController(V3ResultReopenService service,ObjectMapper mapper){
    ...
    @PostMapping(path="/api/v3/mobile/results/{resultUuid}/reopen",consumes=MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<ApiResponse<V3LifecycleAck>> reopen(@AuthenticationPrincipal V3AuthenticatedUser user,@PathVariable String resultUuid,HttpServletRequest request){
        ...
        try{
            ...
            input=mapper.readValue(bytes,V3ReopenRequest.class);
        }catch(IOException | IllegalArgumentException e){throw new V3AuthException("VALIDATION_FAILED","Use the explicit 3.0 reopen JSON contract.",HttpStatus.UNPROCESSABLE_ENTITY);}
        return ResponseEntity.ok().cacheControl(CacheControl.noStore()).body(ApiResponse.success("Reopen acknowledged; retrieve current result state for reconciliation.",service.reopen(user,resultUuid,input)));
    }
    private V3AuthException oversized(){return new V3AuthException("PAYLOAD_TOO_LARGE","Reopen JSON exceeds 32 KiB.",HttpStatus.PAYLOAD_TOO_LARGE);}
}
