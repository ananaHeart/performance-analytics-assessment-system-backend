...
### `competency_tags`
Stores competency references per grade level and subject. These competencies are used when building test parts and analytics groupings.
...
- A `test_part` belongs to `test_id` and `competency_id`
